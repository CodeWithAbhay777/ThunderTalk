...
1. Copy `.env.example` to `.env.local` and set every value. `AUTH_PASSWORD_BCRYPT_HASH` must be the complete 60-character value produced by `bcrypt.hash(password, 12)`; `12` is its work factor. In a Next `.env` file, escape each dollar sign as `\$` so environment-variable expansion does not corrupt the hash. Use a random `JWT_SECRET` of at least 32 bytes and a stable random PBKDF2 salt.
2. Start MongoDB with a dedicated account and TLS in any remote deployment.
3. Run `npm run dev`.

The login password is checked by the API only to obtain an HTTP-only session cookie. After successful login the browser derives a non-extractable AES key using PBKDF2-SHA-256 (600,000 iterations) and the configured public salt. Each save produces a new 96-bit random AES-GCM IV.

Do not change `NEXT_PUBLIC_DIARY_KDF_SALT_BASE64` after creating entries: it is public but essential for decrypting them. Keep regular database backups; encryption intentionally makes password recovery impossible.
