...
const encryptedUpdate = z.object({ encryptedContent: z.string().min(1).max(2_000_000), iv: z.string().min(16).max(64) });
...
export async function GET(request: NextRequest, context: Context): Promise<NextResponse> {
  ...
  try { await connectDatabase(); const entry = await Diary.findOne({ date }).lean(); if (!entry) return NextResponse.json({ error: "Diary entry not found." }, { status: 404 }); return NextResponse.json({ date: entry.date, encryptedContent: entry.encryptedContent, iv: entry.iv, createdAt: entry.createdAt, updatedAt: entry.updatedAt }); } catch (error) { return errorResponse(error); }
}
...
export async function PUT(request: NextRequest, context: Context): Promise<NextResponse> {
  ...
  try { await connectDatabase(); const entry = await Diary.findOneAndUpdate({ date }, parsed.data, { new: true, runValidators: true }).lean(); if (!entry) return NextResponse.json({ error: "Diary entry not found." }, { status: 404 }); return NextResponse.json({ date: entry.date, encryptedContent: entry.encryptedContent, iv: entry.iv, createdAt: entry.createdAt, updatedAt: entry.updatedAt }); } catch (error) { return errorResponse(error); }
}
