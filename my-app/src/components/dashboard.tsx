...
import { decryptDiaryContent, deriveDiaryKey, deriveLegacyScopedDiaryKey } from "@/crypto/diary-crypto";
...
export function Dashboard() {
  ...
  const clearEntryKey = useCallback(() => { setKey(null); setContent(null); }, []);
  ...
  async function unlockEntry(password: string) {
    ...
    const scopedKey = await deriveDiaryKey(password, date);
    if (!entry) { setKey(scopedKey); setContent(""); setDialogOpen(false); return; }
    try {
      setContent(await decryptDiaryContent(scopedKey, entry.encryptedContent, entry.iv)); setKey(scopedKey);
    } catch {
      try {
        const renamedKey = await deriveLegacyScopedDiaryKey(password, date);
        setContent(await decryptDiaryContent(renamedKey, entry.encryptedContent, entry.iv)); setKey(scopedKey);
      } catch {
        // Entries created before date-scoped keys remain readable. Saving one
        // re-encrypts it with the current Thunder Talk date-scoped key.
        const legacyKey = await deriveDiaryKey(password);
        setContent(await decryptDiaryContent(legacyKey, entry.encryptedContent, entry.iv)); setKey(scopedKey);
      }
    }
    setDialogOpen(false);
  }
  ...
  return <main className="app-shell"><header><div><a className="brand" href="/dashboard">Thunder Talk</a><span className="muted">Encrypted per entry</span></div><button className="ghost" onClick={() => void logout()}>Lock diary</button></header>
    <div className="workspace"><aside className="sidebar"><div className="sidebar-row"><h2>Entries</h2><button className="icon-button" aria-label="New entry" onClick={() => selectDate(localDate())}>+</button></div><label className="sr-only" htmlFor="date-search">Find date</label><input id="date-search" type="date" value={date} onChange={(event) => selectDate(event.target.value)} />
      ...
      <section className="content"><div className="content-title"><div><p className="eyebrow">Daily entry</p><h1>{new Date(`${date}T12:00:00`).toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric", year: "numeric" })}</h1></div><button className="danger" onClick={async () => { if (entry && confirm("Permanently delete this encrypted entry?")) { await diaryApi.remove(date); clearEntryKey(); setEntry(null); await refreshIndex(); } }} disabled={!entry}>Delete</button></div>
        {notice && <p className="error" role="alert">{notice}</p>}{loading ? <p className="muted">Loading encrypted entry…</p> : encryptedPreview}{!loading && !entry && !key && <section className="encrypted-preview"><p className="eyebrow">No ciphertext yet</p><p className="muted">Unlock this date to create its encrypted entry.</p><button className="button" onClick={() => setDialogOpen(true)}>Unlock new entry</button></section>}{key && content !== null && <DiaryEditor key={date} date={date} initialContent={content} exists={Boolean(entry)} cryptoKey={key} onSaved={() => void refreshIndex()} onDirtyChange={onDirtyChange} />}
